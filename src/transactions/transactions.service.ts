    ...
    try {
      ...
      if (budget.userId !== userId) {
        throw new ForbiddenException(
          'You do not have permission to add a transaction to this budget',
        );
      }
      ...
      if (category.budgetId !== budgetId) {
        throw new BadRequestException(
          'Category does not belong to the specified budget',
        );
      }
    ...
    if (updateTransactionDto.budgetId || updateTransactionDto.categoryId) {
      ...
      if (categoryId) {
        ...
        const targetBudgetId =
          budgetId || (await this.findOne(id, userId)).budgetId;
